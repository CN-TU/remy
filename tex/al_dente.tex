...
%  \tikzstyle{every label}=[blue!75]
  ...
  \path[->] (s1) edge node {} (s2);
    ...
    \node [place] (s3) [right of=s2, label={[align=center]below:\nnfontsize LSTM (32)}] {};

  \path[->] (s2) edge node {} (s3); 
  \path[->] (s3) edge [loop above] node {} (s3);   
   
    \node [place] (s4) [right of=s3, label={[align=center]below:\nnfontsize LSTM (32)}] {};    
    
  \path[->] (s3) edge node {} (s4);
  \path[->] (s4) edge [loop above] node {} (s4);
    
    \node [place] (s7) [right of=s4, label={[align=center]below:\nnfontsize Lin.~(32,1)}] {};
