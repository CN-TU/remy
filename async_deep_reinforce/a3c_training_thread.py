...
class A3CTrainingThread(object):
  def __init__(self,
    ...
    logging.info(" ".join(map(str,("delay_delta", delay_delta, "cooperative", cooperative))))
  ...
  def inverse_sigmoid(self, x):
    return (1+math.exp(-self.delay_delta))/(1+math.exp(SIGMOID_ALPHA*(x-self.delay_delta)))
  ...
  def process(self, sess, global_t, summary_writer, summary_op, summary_inputs, time_difference=None):
    ...
    if not (np.isfinite(R_duration) and R_duration > 0):
      print("R_duration", R_duration)
    assert(np.isfinite(R_duration) and R_duration > 0)
    ...
    # if final:
      ...
      if self.episode_reward_throughput > 0:
        ...
        things = {
          ...
          "v_estimated_throughput": values[0][0]/(1/values[0][1]),
          "v_estimated_loss_rate": (values[0][2] - values[0][0])/values[0][2],
          "v_duration": values[0][1],
          "v_packets": values[0][0],
          "v_sent": values[0][2],
