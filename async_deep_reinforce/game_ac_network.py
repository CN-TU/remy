...
class GameACLSTMNetwork(GameACNetwork):
	def __init__(self,
		...
		with tf.device(self._device), tf.variable_scope(scope_name) as scope:
			...
			# weight for policy output layer
			self.W_hidden_to_action_mean_fc, self.b_hidden_to_action_mean_fc = self._fc_variable([HIDDEN_SIZE, ACTION_SIZE], 50, 60)
			self.W_hidden_to_action_var_fc, self.b_hidden_to_action_var_fc = self._fc_variable([HIDDEN_SIZE, ACTION_SIZE], 1, 2)
			...
			raw_pi_scale = tf.matmul(lstm_outputs, self.W_hidden_to_action_var_fc) + self.b_hidden_to_action_var_fc
			# pi_mean = tf.concat([tf.slice(raw_pi_mean,(0,0),(-1,2)), tf.nn.softplus(tf.slice(raw_pi_mean,(0,2),(-1,-1)))], axis=1)
			# pi_mean = raw_pi_mean
			# policy (output)
			# TODO: Now the network is completely linear. And can't map non-linear relationships
			self.pi = (
				# tf.nn.softplus(raw_pi_loc), # mean
				# tf.nn.softplus(raw_pi_scale) #var
				tf.clip_by_value(tf.nn.softplus(raw_pi_loc), tiny, float("inf")), # mean
				tf.clip_by_value(tf.nn.softplus(raw_pi_scale), tiny, float("inf")) #var
			)

			self.gamma = tf.contrib.distributions.Gamma(self.pi[0], self.pi[1], allow_nan_stats=False, validate_args=True)
			self.chosen_action = tf.clip_by_value(self.gamma.sample(), 0.1, float("inf"))
			# value (output)
			v_ = tf.matmul(h_fc, self.W_hidden_to_value_fc) + self.b_hidden_to_value_fc
			self.v = tf.reshape( v_, [-1] )
