...
public:
  WhiskerImprover( const Evaluator<  WhiskerTree > & evaluator, const WhiskerTree & unicorn, const WhiskerImproverOptions & options,
                   const double score_to_beat )
    : ActionImprover< WhiskerTree, Whisker >( evaluator, unicorn, score_to_beat),
      _options( options ) {};
};
